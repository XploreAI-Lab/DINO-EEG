...
def find_width_from_same_eeg(target_filename: str, width_mapping: Dict[str, int]) -> int:
    ...
    if '_' in target_filename:
        ...
        if len(parts) >= 3:
            eeg_prefix = '_'.join(parts[:3])  # 取前3部分作为脑电段标识
...
def convert_slice_data_to_original(input_file: str, tusz_file: str, output_file: str):
    ...
    参数:
    - input_file: 输入的切片数据文件路径
    - tusz_file: 保留参数以兼容现有调用（不使用）
    - output_file: 输出文件路径
    """
    # 加载切片数据
    print("正在加载切片数据...")
    ...
    for item in slice_data:
        try:
            ...
            slice_width = item.get('width', 3600)  # 默认切片宽度为200
    ...
    # 对每个image_id的检测结果进行NMS处理（阈值为0）
    print("\n正在对每个image_id进行NMS处理...")
    image_groups = {}
    ...
    for image_id, detections in image_groups.items():
        # 对当前image_id的所有检测结果应用NMS（阈值为0）
        nms_detections = apply_nms(detections, iou_threshold=0.0)
        final_data.extend(nms_detections)
        
        removed_count = len(detections) - len(nms_detections)
        nms_removed_count += removed_count
        
        if removed_count > 0:
            print(f"  {image_id}: 移除了 {removed_count} 个重叠检测框")
    
    print(f"NMS处理完成，总共移除了 {nms_removed_count} 个重叠检测框")
    print(f"最终保留 {len(final_data)} 个检测结果")
    ...
    print(f"NMS移除: {nms_removed_count} 条重叠数据")
    print(f"最终输出: {len(final_data)} 条数据")
    print(f"输出文件: {output_file}")

if __name__ == "__main__":
    # 文件路径
    input_file = r"d:\python\dino_0917\Siena3600_results.bbox (4).json"
    tusz_file = r"d:\python\dino_0917\anno_siena.json"
    output_file = r"d:\python\dino_0917\converted_Siena3600_n.json"
