...
class IntegratedEvaluator:
    ...
    def json_to_tsv(self, gt_json_path: str, pred_json_path: str, meta_json_path: Optional[str], 
                   ...
                   score_threshold: float = 0.3, max_predictions: int = 18, channelwise: bool = False) -> None:
        ...
        if channelwise:
            print("检测到通道级别处理模式，按段+通道生成TSV...")
            gt_by_seg = defaultdict(list)
            gt_by_chan = defaultdict(list)
            for ann in gt_data:
                h5_name = ann['image_id']
                base_id = os.path.splitext(h5_name)[0]
                parts = base_id.split('_')
                seg_id = '_'.join(parts[:3])
                gt_by_seg[seg_id].append(ann)
                if len(parts) >= 4:
                    ch = parts[-1]
                    gt_by_chan[(seg_id, ch)].append(ann)
            ...
            for pred in pred_data:
                ...
                parts = base_id.split('_')
                seg_id = '_'.join(parts[:3])
                ch = parts[-1] if len(parts) >= 4 else ""
                channels_set.add(ch)
                hyp_predictions[(seg_id, ch)].append(pred)
            target_keys = list(seg_to_width.keys())
        else:
        ...
        if channelwise:
            channels_set = set()
            for pred in pred_data:
                base_id = os.path.splitext(pred['image_id'])[0]
                parts = base_id.split('_')
                if len(parts) >= 4:
                    channels_set.add(parts[-1])
            for seg_id in target_keys:
                width = seg_to_width[seg_id]
                for ch in channels_set:
                    base_name = f"{seg_id}_{ch}_events.tsv"
                    ...
                    with open(gt_path, 'w', encoding='utf-8') as f:
                        ...
                        if (seg_id, ch) in gt_by_chan:
                            for ann in gt_by_chan[(seg_id, ch)]:
                                bbox = ann['bbox']
                                onset = bbox[0]
                                duration = bbox[2]
                                line = f"{onset:.2f}\t{duration:.2f}\tsz\t1.00\tn/a\t2014-01-01 00:00:00\t{width:.2f}\n"
                                f.write(line)
                        elif seg_id in gt_by_seg:
                            for ann in gt_by_seg[seg_id]:
                                bbox = ann['bbox']
                                onset = bbox[0]
                                duration = bbox[2]
                                line = f"{onset:.2f}\t{duration:.2f}\tsz\t1.00\tn/a\t2014-01-01 00:00:00\t{width:.2f}\n"
                    ...
                    with open(hyp_path, 'w', encoding='utf-8') as f:
                        f.write("onset\tduration\teventType\tconfidence\tchannels\tdateTime\trecordingDuration\n")
                        preds = hyp_predictions.get((seg_id, ch), [])
                        any_valid = False
                        cnt = 0
                        for pred in preds:
                            cnt += 1
    ...
    def compute_scores(self, tp: int, fp: int, ref_true: int, duration: float) -> Tuple[float, float, float, float]:
        ...
        fp_rate = fp / (duration / 3600 / 24) if duration > 0 else np.nan
    ...
    def evaluate_dataset(self, ref_folder: str, hyp_folder: str, output_file: str, 
                        conf_threshold: float = 0.0, avg_per_subject: bool = True, return_counts: bool = True) -> Dict[str, Any]:
        """评估数据集"""
        ...
        if avg_per_subject:
            ...
            if return_counts:
                for subject in sample_scores.keys():
                    by_subject_counts[subject] = {
                        "sample": {
                            "tp": int(sample_scores[subject]["tp"]),
                            "fp": int(sample_scores[subject]["fp"]),
                            "refTrue": int(sample_scores[subject]["refTrue"]),
                            "duration": float(sample_scores[subject]["duration"])
                        },
                        "event": {
                            "tp": int(event_scores[subject]["tp"]),
                            "fp": int(event_scores[subject]["fp"]),
                            "refTrue": int(event_scores[subject]["refTrue"]),
                            "duration": float(event_scores[subject]["duration"])
                        }
                    }
        else:
            # 累加所有subject
            for total_dict, out_dict in [({"tp":0, "fp":0, "refTrue":0, "duration":0}, sample_results),
                ...
                if return_counts:
                    if out_dict is sample_results:
                        sample_total_counts = {
                            "tp": int(total_dict["tp"]),
                            "fp": int(total_dict["fp"]),
                            "refTrue": int(total_dict["refTrue"]),
                            "duration": float(total_dict["duration"]) 
                        }
                    else:
                        event_total_counts = {
                            "tp": int(total_dict["tp"]),
        ...
        if return_counts:
            output["by_subject_counts"] = by_subject_counts
            if not avg_per_subject:
                output["sample_counts_total"] = sample_total_counts if 'sample_total_counts' in locals() else {"tp":0,"fp":0,"refTrue":0,"duration":0}
                output["event_counts_total"] = event_total_counts if 'event_total_counts' in locals() else {"tp":0,"fp":0,"refTrue":0,"duration":0}
    ...
    def run_quick_evaluation_from_data(self,
        ...
        if not channelwise:
            ...
            for pred in pred_data:
                ...
                channel = parts[-1] if len(parts) >= 4 else ""
                key = (seg_id, channel)
                merged[key].append(pred)
            for (seg_id, channel), boxes in merged.items():
                kept = []
                for box in sorted(boxes, key=lambda x: -x['score']):
                    if box['score'] < 0.01:
                        break
                    ...
                    if not overlap_found:
                        new_box = box.copy()
                        new_box['image_id'] = f"{seg_id}_{channel}"
                        merged_predictions.append(new_box)
                        kept.append(box)
        ...
        for threshold in thresholds:
            ...
            self._json_data_to_tsv(
                gt_data, merged_predictions, meta_json_path,
            ...
            try:
                ...
                event_ses = result.get('event_results', {}).get('sensitivity', np.nan)
                event_fpr = result.get('event_results', {}).get('fpRate', np.nan)
                print(f"    事件级别: ses={event_ses:.4f}, fpr={event_fpr:.4f}, F1={event_f1:.4f}")
    ...
    def _json_data_to_tsv(self, gt_data: List[Dict[str, Any]], pred_data: List[Dict[str, Any]], 
                         ...
                         gt_output_dir: str, hyp_output_dir: str, 
                         score_threshold: float = 0.3, max_predictions: int = 18,
        ...
        if channelwise:
            ...
            for pred in pred_data:
                ...
                hyp_predictions[(seg_id, channel)].append(pred)
            target_keys = list(seg_to_width.keys())
        else:
            is_merged_format = False
            if pred_data:
        ...
        if channelwise:
            ...
            for seg_id in target_keys:
                ...
                for ch in channels_set:
                    ...
                    with open(gt_path, 'w', encoding='utf-8') as f:
                        f.write("onset\tduration\teventType\tconfidence\tchannels\tdateTime\trecordingDuration\n")
                        if seg_id in gt_annotations:
                            for ann in gt_annotations[seg_id]:
                                bbox = ann['bbox']
                                onset = bbox[0]
                                duration = bbox[2]
                                line = f"{onset:.2f}\t{duration:.2f}\tsz\t1.00\tn/a\t2014-01-01 00:00:00\t{width:.2f}\n"
    ...
    def run_quick_evaluation(self, 
        ...
        preds_sample = []
        try:
            preds_sample = self.load_json(pred_json_path)
        except Exception:
            preds_sample = []
        auto_channelwise = False
        if preds_sample:
            sample_id = preds_sample[0].get('image_id', '')
            auto_channelwise = len(os.path.splitext(sample_id)[0].split('_')) >= 4
        use_channelwise = channelwise or auto_channelwise
        if use_channelwise:
            print("检测到通道级或指定的通道模式，跳过多通道合并")
            source_pred_path = pred_json_path
        else:
            print("步骤1: 执行多通道NMS合并（阈值0.01）...")
            ...
            source_pred_path = str(merged_path)
        
        # 步骤2: 对0.1到0.9的9个置信度阈值进行评估
        print("步骤2: 评估不同置信度阈值...")
        thresholds = np.arange(0.1, 0.81, 0.1).tolist()
        ...
        for threshold in thresholds:
            ...
            # 转换为TSV格式
            self.json_to_tsv(
                gt_json_path, source_pred_path, meta_json_path,
                str(gt_dir), str(hyp_dir),
                score_threshold=threshold,
                max_predictions=30,
                channelwise=use_channelwise
            )
            ...
            try:
                ...
                print(f"    评估阈值 {threshold:.2f} 时出错: {e}")
                all_results[f"threshold_{threshold:.2f}"] = {
                    'threshold': threshold,
                    'event_f1': 0.0,
                    'error': str(e)
