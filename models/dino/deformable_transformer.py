...
class DeformableTransformer(nn.Module):
    ...
    # 其中input_query_label, attn_mask和input_query_bbox用于dn detr
    # srcs, masks是nestedtensor depose后的src与mask，poss是backbone（joiner）输出的多层位置编码
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        Input:num_dn/ d_model =? hidden_dim
            ...
            - refpoint_embed: [batch_size, denoise_num_queries, 4]. None in infer
        ...
        Input:num_dn/ d_model =? hidden_dim
            ...
            - refpoint_embed: 新增去噪任务与推理任务的归一化坐标 [batch_size, denoise_num_queries, 4]. 仅在训练中存在
            - pos_embeds: List of multi pos embeds [batch_size, channel, height, width]
        ...
        # (h, w) 保存特征图的形状
        spatial_shapes = []
        # 对于每一层特征 srcs, masks, pos_embeds长度肯定相同
        ...
        # 各尺度特征图中非padding部分的边长占其边长的比例，小数  [bs, 4, 2]  如全是1
        ...
        memory, enc_intermediate_output, enc_intermediate_refpoints = self.encoder(
            ...
            ref_token_index=enc_topk_proposals,  # bs, nq 默认为None
            ref_token_coord=enc_refpoint_embed,  # bs, nq, 4 默认为None
        )
        #########################################################
        # End Encoder
        # - memory: bs, \sum{hw}, c
        ...
        if self.two_stage_type == 'standard':  # 默认使用二阶段,two_stage_type = standard
            # 默认不进入, two_stage_learn_wh = False
            if self.two_stage_learn_wh:  
            ...
            output_memory, output_proposals = gen_encoder_output_proposals(memory, mask_flatten, spatial_shapes,
                                                                           input_hw)
            # [B, K, D] 对encoder输出进行处理：全连接层 + LayerNorm
            ...
            # [B, K, 4] 还原所有检测框  # two-stage 必须和 iterative bounding box refinement一起使用 不然bbox_embed=None 报错
            ...
            # torch.max返回一个长度为2的tuple，[0]代表最大值，[1]为最大值的下标；topk也返回一个长度为2的tuple，且同理
            # max(-1)[0].shape = [B, K] 表示每个点取置信度最大的每个类的分数 topk表示再在这些点中取前topk个分类分数最大的点
            try:
                topk_proposals = torch.topk(enc_outputs_class_unselected.max(-1)[0], topk, dim=1)[1]  # bs, nq
            except RuntimeError:
                print(enc_outputs_class_unselected.shape)
                exit(1)
            ...
            if self.embed_init_tgt:  # 默认进入  # 实际上就是随机初始化了tgt_embed 作为content query部分
                # tgt_是正常的，tgt是输入的denoising的tgt
                tgt_ = self.tgt_embed.weight[:, None, :].repeat(1, bs, 1).transpose(0, 1)  # [B, NQ, d]
            else:  # 默认不进入 这个才是使用先验框对应的初始化的content
                tgt_ = tgt_undetach.detach()

            # 如果有denoising的refpoint_embed
            if refpoint_embed is not None:  # 训练时进入
                # 将正常query与cdn正负情绪query的bbox拼接200+900
                refpoint_embed = torch.cat([refpoint_embed, refpoint_embed_],
                                           dim=1)
                # 将初始化query与cdn正负情绪query拼接200+900 denoising的query在前面
        ...
        hs, references = self.decoder(
            ...
            pos=lvl_pos_embed_flatten.transpose(0, 1),  # backbone的位置编码
            refpoints_unsigmoid=refpoint_embed.transpose(0, 1), # 先验框
            level_start_index=level_start_index,
            spatial_shapes=spatial_shapes,
            valid_ratios=valid_ratios,
            tgt_mask=attn_mask)
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, encoder_layer, num_layers, norm=None, d_model=256,
                 num_queries=300,
                 deformable_encoder=False, # 默认为True
                 enc_layer_share=False, # 默认为False
                 enc_layer_dropout_prob=None, # 默认为None
                 two_stage_type='no',  # ['no', 'standard', 'early', 'combine', 'enceachlayer', 'enclayer1']
                 ):
        super().__init__()
        # prepare layers
        ...
        # DINO中默认为standard或no，不会出现'enceachlayer', 'enclayer1'
        self.two_stage_type = two_stage_type
    ...
    def get_reference_points(spatial_shapes, valid_ratios, device):
        ...
        # 遍历4个特征图的shape  比如 H_=100  W_=150 每一个像素点都是一个参考点
        for lvl, (H_, W_) in enumerate(spatial_shapes):
            ...
            # 得到每一层feature map对应的reference point，即ref，shape为[B, flatten_W*flatten_H, 2] 每一项都是xy [B, WH, 2]
            ref = torch.stack((ref_x, ref_y), -1)
            reference_points_list.append(ref)
        # list4: [bs, H/8*W/8, 2] + [bs, H/16*W/16, 2] + [bs, H/32*W/32, 2] + [bs, H/64*W/64, 2] ->
        # 将所有尺度的feature map对应的reference point在第一维合并，得到[B, K, 2] [bs, H/8*W/8+H/16*W/16+H/32*W/32+H/64*W/64, 2]
        ...
        # [B, K, num_level, 2] 每一个特征层的每个参考点坐标，换句话说就是每个特征点有4个参考点 这是因为每个点有4层特征
    ...
    def forward(self,
                ...
                ref_token_index: Optional[Tensor] = None, # 一开始是None
                ref_token_coord: Optional[Tensor] = None # 一开始是None
                ):
        """
        Input:
        ...
        """
        """
        src: 多尺度特征图(4个flatten后的特征图) [B, K, C] [bs, H/8 * W/8 + H/16 * W/16 + H/32 * W/32 + H/64 * W/64, 256]
        ...
        # main process 对于每一层的encoder layer
        for layer_id, layer in enumerate(self.layers):
            # main process
            dropflag = False  # if not dropflag 一起删除 从未被修改,一定进入if语句
            # 未进入 默认enc_layer_dropout_prob=None
            ...
            if not dropflag:  # 一定进入
                # 一定使用deformable_encoder 默认为True
                if self.deformable_encoder:
            ...
            # 在dino中two_stage_type只会为standard或no，因此不会进入
            ...
            # aux loss 在dino中，默认不会进入,因为ref_token_index一定为None
        ...
        # [bs, H/8 * W/8 + H/16 * W/16 + H/32 * W/32 + H/64 * W/64, 256]
        # [B, K, C] 输出经过6层自注意力层后的特征图 None None
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        """
        """
        tgt: [900 + 200, bs, 256] 预设的query embedding 即content query
        memory: [H/8 * W/8 + H/16 * W/16 + H/32 * W/32 + H/64 * W/64, bs, 256] 来自backbone的多层特征信息 encoder的输出
        ...
        pos: [H/8 * W/8 + H/16 * W/16 + H/32 * W/32 + H/64 * W/64, bs, 256] 来自backbone的位置信息 原始位置信息，区别于decoder query的位置编码
        refpoints_unsigmoid: [1100, 1, 4] query的参考点 即decoder query的位置编码 position query
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            # 预处理参考点 默认不进入,decoder_query_perturber=None
            ...
            if not dropflag:  # 默认进入
                output = layer(
                    ...
                    tgt_query_sine_embed=query_sine_embed,  # 位置编码的Embedding 没用
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
                 ...
                 dropout=0.1, activation="relu", # dropout默认是0
                 n_levels=4, n_heads=8, n_points=4,
                 add_channel_attention=False, # 默认为False
                 use_deformable_box_attn=False, # 默认为False
                 box_attn_type='roi_align', # 默认为roi_align，也没用别的选项
                 ):
        super().__init__()
        # self attention
        if use_deformable_box_attn:
            self.self_attn = MSDeformableBoxAttention(d_model, n_levels, n_heads, n_boxes=n_points, # type: ignore
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, key_padding_mask=None):
        ...
        # [bs, H/8 * W/8 + H/16 * W/16 + H/32 * W/32 + H/64 * W/64, 256]
        # [B, K, C] encoder layer输出经过deformable自注意力卷积的东西
        return src
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        super().__init__()
        # 依次['sa', 'ca', 'ffn']forward
        self.module_seq = module_seq
        assert sorted(module_seq) == ['ca', 'ffn', 'sa']
        # cross attention
        if use_deformable_box_attn:
            self.cross_attn = MSDeformableBoxAttention(d_model, n_levels, n_heads, n_boxes=n_points,    # type: ignore
                                                       used_func=box_attn_type)
        else:
    ...
    def forward_ca(self,
                   # for tgt
                   tgt: Optional[Tensor],  # nq, bs, d_model
                   tgt_query_pos: Optional[Tensor] = None,  # pos for query. MLP(Sine(pos)) 位置编码化的参考点
                   tgt_query_sine_embed: Optional[Tensor] = None,  # pos for query. Sine(pos)
                   tgt_key_padding_mask: Optional[Tensor] = None,
                   tgt_reference_points: Optional[Tensor] = None,  # nq, bs, 4 没有经过位置编码的参考点
        ...
        # 所以q=query embedding + query pos（即第一个参数）, k = query pos通过一个全连接层->2维, v=上一层输出的output（即memory）
    ...
    def forward(self,
                ...
                tgt: Optional[Tensor],  # nq, bs, d_model
                tgt_query_pos: Optional[Tensor] = None,  # pos for query. MLP(Sine(pos))
                tgt_query_sine_embed: Optional[Tensor] = None,  # pos for query. Sine(pos) 完全没用上
                tgt_key_padding_mask: Optional[Tensor] = None,
                tgt_reference_points: Optional[Tensor] = None,  # nq, bs, 4
