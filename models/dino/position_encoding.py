...
class PositionEmbeddingSine1DW(nn.Module):
    """
    This is a more standard version of the position embedding, very similar to the one
    used by the Attention is all you need paper, generalized to work on images.
    """
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        # [B, H, W, num_pos_feats, 2] -> [B, H, W, 256]
        pos_x = torch.stack(
            (pos_x[:, :, :, 0::2].sin(), pos_x[:, :, :, 1::2].cos()), dim=4
        ).flatten(3)
        # [B, 256, H, W]
        return pos_x.permute(0, 3, 1, 2)


class PositionEmbeddingSine1DH(nn.Module):
    def __init__(self, num_pos_feats=64, temperature=10000, normalize=False, scale=None):
        super().__init__()
        self.num_pos_feats = num_pos_feats
        self.temperature = temperature
...
def build_position_encoding(args):
    ...
    if args.position_embedding in ("v2", "sine"):
        ...
        #     normalize=True,
        # )
        position_embedding = PositionEmbeddingSine1DW(
            N_steps,
            temperature=args.pe_temperatureW,
            normalize=True,
        )
