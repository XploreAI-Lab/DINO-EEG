...
def handle(path: str, do_filter: bool = True):
    # with open(os.path.join(path, task + "Set.txt"), "r") as txt:
...
def main(path):
    for task in tasks:
        if task != "eval":
            seiz, noseiz = handle(os.path.join(path, "stft_amp_w_scale_w_crop", task), True)
            with open(
            ...
            seiz, noseiz = handle(os.path.join(path, "stft_amp_w_scale_w_crop", task), False)
            with open(
