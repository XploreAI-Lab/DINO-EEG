...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor]):
    ...
    if tensor_list[0].ndim == 3:
        ...
        # c, l = tensor_list[0].shape
        c = tensor_list[0].shape[0]
        l = get_longest_l(tensor_list)
        b = len(tensor_list)
        dtype = tensor_list[0].dtype
        device = tensor_list[0].device
        tensor = torch.zeros((b, c, l), dtype=dtype, device=device)
...
def init_distributed_mode(args):
    ...
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ:
        ...
        print(
            "Distributed training detected: world_size: {}, rank: {}, local_rank: {}".format(
        ...
        args.gpu = args.local_rank = int(os.environ["LOCAL_RANK"]) if "LOCAL_RANK" in os.environ else args.local_rank
        args.rank = args.rank * local_world_size + args.local_rank
        print(
            "Legacy distributed training: world size: {}, rank: {}, local rank: {}".format(
                args.world_size, args.rank, args.local_rank
            )
        )
...
class NestedTensor_MST(object):
    def __init__(
        self, tensors, mask: Optional[Tensor], ch_names: Optional[list] = None
    ):
        self.tensors = tensors
        self.mask = mask
    ...
    def to(self, device):
        ...
        if mask is not None:
            ...
            cast_mask = None
        return NestedTensor_MST(cast_tensor, cast_mask, self.ch_names)
...
def nested_tensor_from_tensor_list_mst(
    tensor_list: List[Tensor], chann_list: List[str]
):
    # TODO make this more general
    if tensor_list[0].ndim == 3:
        if torchvision._is_tracing():
