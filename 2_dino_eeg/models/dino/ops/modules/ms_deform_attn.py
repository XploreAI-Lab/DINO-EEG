...
class MSDeformAttn(nn.Module):
    def __init__(self, d_model=256, n_levels=4, n_heads=8, n_points=4):
        ...
        # 每个query对应的所有采样点（每个头x每一层x每个点）的注意力权重  n_heads * n_levels * n_points = 8x4x4=128
    ...
    def _reset_parameters(self):
        ...
        # [n_heads, n_levels, n_points, xy] = [8, 4, 4, 2] 每个头每一层每个采样点初始化的坐标
        # 绕了半天实际效果就是初始化为(-k, -k), (-k, 0) , ..., (k, k)
        ...
        # 每个头负责一个初始偏移方向（坐标），不同的采样点的偏移方向长度i不同
        for i in range(self.n_points):
            # [8,4,4,2] *=改的是最后一维 第0个点，它的坐标偏移1，第二个点它的坐标偏移2...
            # 第一个头的4个点的坐标为(1,0),(2,0),(3,0),(4,0)即(i,0)
            grid_init[:, :, i, :] *= i + 1
        with torch.no_grad():
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        ...
        """
        """
        【encoder】 4个特征层，每个特征层一个参考点，共4个参考点（或说一个参考点有4层），每个参考点对应8个头的4个采样点
        query: 4个flatten后的特征图+4个flatten后特征图对应的位置编码 = src_flatten + lvl_pos_embed_flatten
               [bs, H/8 * W/8 + H/16 * W/16 + H/32 * W/32 + H/64 * W/64, 256] [B, K, C]
        reference_points: 4个flatten后特征图对应的归一化参考点坐标 每个特征点有4个参考点 xy坐标
                          [bs, H/8 * W/8 + H/16 * W/16 + H/32 * W/32 + H/64 * W/64, 4, 2] [B, K, 2/4]
        ...
        # 注意力机制的V 输入不包含位置编码的input_flatten映射而成
        ...
        # [bs, Len_q, 8, 16] -> [bs, Len_q, 8, 16] -> [bs, Len_q, 8, 4, 4] 通过softmax进行0~1化
        ...
        if reference_points.shape[-1] == 2:  # one stage
            ...
            # 参考点 + 偏移量/特征层宽高 = 采样点 sampling_offsets学习的是坐标，除以offset_normalizer学习到0~1之间的东西
            ...
            # 前面一阶段reference_points是xy坐标，现在两阶段reference_points[..., :2]是xy [2:]是wh
