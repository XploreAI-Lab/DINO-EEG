...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # [3, 4]  concat all box [NT, 4] 一个B中所有的真实框的数量NT
        ...
        # out_bbox[B*NQ, 4] tgt_bbox[NT, 4] -> [B*NQ, NT]
        ...
        # [B, _NT] 一个B中，不同图片真实框的数量，可能为0
        sizes = [len(v["boxes"]) for v in targets]

        # 匈牙利算法进行二分图匹配  从100个预测框中挑选出最终的3个预测框 分别和gt计算损失  这个组合的总损失是最小的
        # 0: [3]  5, 35, 63   匹配到的gt个预测框idx
        # 1: [3]  1, 0, 2     对应的gt idx
        # indices长度为B的list，每个元素是一个长度为2的tuple，tuple的元素是一个ndarray
        # ndarray的长度取决于sizes对应的_NT，可能为0，tuple的左右ndarray长度相等
        # 左边是匹配到的NQ的下标，右边则是匹配到的NT的下标，代表最优的匹配
        # 也就是说，一定会匹配到NT个
