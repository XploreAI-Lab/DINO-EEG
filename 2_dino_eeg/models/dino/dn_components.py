...
def prepare_for_cdn(
    dn_args, training, num_queries, num_classes, hidden_dim, label_enc, device
):
    """
    ...
    if training:  # 如果是训练
        ...
        known = [(torch.ones_like(t["labels"])).cuda(device) for t in targets]
        # 批量大小
        batch_size = len(known)
        # 该 batch 中各图片的目标数量
        # 每张图片的gt数量的列表
        known_num = [sum(k) for k in known]
        if int(max(known_num)) == 0:  # 如果没有目标
            dn_number = 1
        else:  # 如果有目标
            if dn_number >= 100:
                # 一半检测积极,一半检测消极 这里变成了CDN group的数量（N/2n）
                # dn_number = dn_number // (int(max(known_num) * 2))
                # N/4n 1n做gt 3n做nt
                dn_number = dn_number // (int(max(known_num) * Ababab))
            elif dn_number < 1:
                dn_number = 1
        if dn_number == 0:
            dn_number = 1
        ...
        # [4*grp*NT_b, ] (2 * dn_number * num_gts_batch,)
        known_labels = labels.repeat(Ababab * dn_number, 1).view(-1)
        known_indice = known_indice.repeat(Ababab * dn_number, 1).view(-1)
        known_bid = batch_idx.repeat(Ababab * dn_number, 1).view(-1)
        ...
        # [4*grp*NT_b, 4]
        known_bboxs = boxes.repeat(Ababab * dn_number, 1)

        # 用於在 gt labels上加噪
        known_labels_expaned = known_labels.clone().to(torch.int64)
        # 用於在 gt boxes 上加噪
        ...
        pad_size = int(single_pad * Ababab * dn_number)
        # [grp, 4*grp*NT_b]
        positive_idx = (
            torch.tensor(range(len(boxes))) # [0, 1, 2, ..., len(boxes)-1]
            .long()
            .cuda(device)
            .unsqueeze(0)
            .repeat(dn_number, 1)
        ...
        # )
        positive_idx += (
            (torch.tensor(range(dn_number)) * len(boxes) * Ababab)
            .long()
            .cuda(device)
            .unsqueeze(1)
        )
        ...
        if box_noise_scale > 0:
            ...
            rand_part *= rand_sign
            known_bbox_ = (
                known_bbox_ + torch.mul(rand_part, diff).cuda(device) * box_noise_scale
            )
        ...
        m = known_labels_expaned.long().cuda(device)
        ...
        # 将以上“扩展”到所有去噪组
        padding_label = torch.zeros(pad_size, hidden_dim).cuda(device)
        padding_bbox = torch.zeros(pad_size, 4).cuda(device)
        ...
        # map in order
        map_known_indice = torch.tensor([]).cuda(device)
        if len(known_num):
            ...
            # ).long()
            map_known_indice = torch.cat(
                [map_known_indice + single_pad * i for i in range(Ababab * dn_number)]
            ).long()
        if len(known_bid):
        ...
        # (i,j) = True 代表 i 不可見 j
        attn_mask = torch.ones(tgt_size, tgt_size).cuda(device) < 0
        ...
        for i in range(dn_number):
            if i == 0:
                attn_mask[
                    single_pad * Ababab * i : single_pad * Ababab * (i + 1),
                    single_pad * Ababab * (i + 1) : pad_size,
                ] = True
            if i == dn_number - 1:
                attn_mask[
                    single_pad * Ababab * i : single_pad * Ababab * (i + 1), : single_pad * i * Ababab
                ] = True
            else:
                attn_mask[
                    single_pad * Ababab * i : single_pad * Ababab * (i + 1),
                ...
                attn_mask[
                    single_pad * Ababab * i : single_pad * Ababab * (i + 1), : single_pad * Ababab * i
                ] = True
