...
class DINO(nn.Module):
    ...
    def __init__(
        ...
        if num_feature_levels > 1:
            ...
            for _ in range(num_backbone_outs):  # 3个1x1conv
                ...
                input_proj_list.append(
                    nn.Sequential(  # conv1x1  -> 256 channel
        ...
        self.backbone = (
            backbone  # backbone Joiner  0 Backbone + 1 PositionEmbeddingSine
        )
    ...
    def forward(self, samples: NestedTensor, targets: List = None):
        ...
        # 如果尺度类别不够,再通过降维获得新的尺度H,W再减半[1,2048,H/64,W/64] 一般不会出现这种情况?4层特征最后一层是这个
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        # 注意这个src_logits.shape[2]+1，是为了下一步scatter_时使用的，便于能够处理ID“NC+1”，否则会出错
        # [B, NQ, num_classes+1] 初始化为0
        ...
        # 这个地方取[:, :, :-1]，又是忽略掉了id NC+1
        # [B, NQ, num_classes]
        target_classes_onehot = target_classes_onehot[:, :, :-1]
        # 分类损失 = 正样本 + 负样本
        # 原始detr是使用的cross_entropy损失函数，dino这里用的sigmoid_focal_loss
        # cross_entropy函数本质为softmax+log+NLLLoss，取了softmax，类别互斥
        # 但是sigmoid_focal_loss类别不互斥，每个框预测属于该类还是no object，无需显式的定义一个维度来预测no object
        loss_ce = (
            sigmoid_focal_loss(
                src_logits,
        ...
        if self.training and dn_meta and "output_known_lbs_bboxes" in dn_meta:
            ...
            for i in range(len(targets)):
                if len(targets[i]["labels"]) > 0:
                    t = (
                        torch.range(0, len(targets[i]["labels"]) - 1)
                        .long()
                        .cuda(device)
                    )
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes, not_to_xyxy=False, test=False):
        ...
        # prob.view(out_logits.shape[0], -1): [B, NQ*NC] 选择NS个置信度最大的预测
        # topk_values:0~1; topk_indexes:0~NQ*NC
        topk_values, topk_indexes = torch.topk(
            prob.view(out_logits.shape[0], -1), num_select, dim=1
        )
        # [B, NS]
        scores = topk_values
        # [B, NS] out_logits.shape[2]: NC topk_boxes:index//NC 属于哪个query（下标0~NQ）
        topk_boxes = topk_indexes // out_logits.shape[2]
        # [B, NS] 同理 属于哪个label（下标0~NC）
        ...
        # boxes:[B, NQ, 4] topk_boxes.unsqueeze(-1):[B, NS, 1] repeat:[B, NS, 4] 为query的下标0~NQ
        # 结果boxes变为[B, NS, 4] boxes[i,j,k]=boxes[i,index[i,j,k],k] 把0~NQ的下标映射为0~NS
        boxes = torch.gather(boxes, 1, topk_boxes.unsqueeze(-1).repeat(1, 1, 4))
        # [B, NS, 2]
        # boxes = torch.gather(boxes, 1, topk_boxes.unsqueeze(-1).repeat(1, 1, 2))

        # boxes: [B, NS, 4]
        x0, _, x1, _ = boxes.unbind(-1)
        # boxes: [B, num_select, 2] 先简单粗暴地舍弃掉y轴
        boxes = torch.stack([x0, x1], dim=-1)
        ...
        # 归一化坐标 -> 绝对位置坐标(相对于原图的坐标)  [B, num_select, 2]
        ...
        # list: 长度为B    每个list都是一个dict  包括'scores'  'labels'  'boxes'三个字段
        # scores = Tensor[NS,]  这张图片预测的NS个预测框概率分数
        # labels = Tensor[NS,]  这张图片预测的NS个预测框所属类别（下标）
        # 在DINO中是包含背景类的，背景类在gt中不包含，因此在evaluate的代码中被‘排除’
        # boxes = Tensor[NS, 2] 这张图片预测的NS个预测框的绝对位置坐标(相对这张图片的原图大小的坐标)
        return results
