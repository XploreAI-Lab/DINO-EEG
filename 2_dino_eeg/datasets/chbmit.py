...
class CHBMITDataSet(Dataset):
    def __init__(self, task, args):
        super(CHBMITDataSet, self).__init__()

        self.data_dir = os.path.join(args.data_dir, task)
        self.txt_dir = args.chbmit_txt_dir
        ...
        self.data = self._downsample(
            ...
            ratio=args.chbmit_downsample_times if task != "eval" else 0,
        )
        # self.data = df_s
...
def build_chbmit_dataloader(collate_fn, args):
    dataloaders = {}
    datasets = {}

    dataset_train = CHBMITDataSet("train", args)
    dataset_validation = CHBMITDataSet("dev", args)
    dataset_test = CHBMITDataSet("eval", args)
